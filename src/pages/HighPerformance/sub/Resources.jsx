...
import { getHighResource } from '../../../service/api';
import Comingsoon from '../../../components/Comingsoon';
import { BASE_URL } from '../../../service/config';
function ResourcesHigh() {
  const [resources, setresources] = useState([]);
  const [htmlString, setHtmlString] = useState('');

  const fetchresources = async () => {
    const result = await getHighResource()
      ...
      <div>
        ...
        {resources.coverImageUrl && resources.coverImageUrl !== "" && resources.coverImageUrl !== "null" && (
          <div className='w-full'>
            <img className='object-cover w-full' src={BASE_URL+`${resources.coverImageUrl}`} alt='' />
          </div>
        )}
