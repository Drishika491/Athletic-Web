...
function SubTechnicalOfficial() {
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      ...
      const homeSubMenus = menuData.find(menu => menu.name === "Technical Officials").publicSubMenus;
              ...
              <div className="flex justify-between items-center xl:items-end space-x-5">
                <div className="hidden xl:flex items-center space-x-5">
                  ...
                  {menuItems.map((menuItem) => (
                    <ul className="navbar-sub dropdown list-none p-0" key={menuItem.name}>
                      <li className="border-primary border-b-[2px] hover:border-white cursor-pointer text-white transition-all text-[0.9rem] 2xl:text-[0.9rem] whitespace-nowrap">
                        <NavLink className="nav-bar-link" to={`/technical-officials/${menuItem.name.toLowerCase().replaceAll("/", "-").replaceAll(" ", "-")}`}>
                          {menuItem.name}
                        </NavLink>
            ...
            <div className="flex items-center gap-3">
              <div className="flex justify-between items-center xl:items-end space-x-5">
                <div className="flex items-center space-x-5">
                  ...
                  {menuItems.map((menuItem) => (
                    <ul className="navbar-sub dropdown list-none p-0" key={menuItem.name}>
                      <li className="hover:border-white border-primary border-b-[2px] cursor-pointer text-white transition-all text-[0.9rem] 2xl:text-[0.9rem] whitespace-nowrap">
                        <NavLink className="nav-bar-link" to={`/technical-officials/${menuItem.name.toLowerCase().replaceAll("/", "-").replaceAll(" ", "-")}`}>
                          {menuItem.name}
                        </NavLink>
...
export default SubTechnicalOfficial;
