...
function SubGetInvolved() {
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      ...
      const homeSubMenus = menuData.find(menu => menu.name === "Get Involved").publicSubMenus;
                  ...
                  {menuItems.map((menuItem) => (
                    <ul className="navbar-sub dropdown p-0" key={menuItem.name}>
                      <li className="border-primary list-none border-b-[2px] hover:border-white cursor-pointer text-white transition-all text-[0.9rem] 2xl:text-[0.9rem] whitespace-nowrap">
                        <NavLink className="nav-bar-link" to={`/get-involved/${menuItem.name.toLowerCase().replaceAll("/", "-").replaceAll(" ", "-")}`}>
                          {menuItem.name}
                        </NavLink>
