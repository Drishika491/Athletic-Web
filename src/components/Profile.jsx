...
function Profile() {
  ...
  const updateUserProfile = async () => {
    const requestBody = {
      Pvid: clud.pvid,
      NickName: clud.nickName,
      FirstName: clud.firstName,
      LastName: clud.lastName,
    };
  console.log('requestBody',requestBody)
    try {
      const response = await fetch(BASE_URL + "Api/IdentityUser/ModifyById", {
        method: "PUT", // Assuming the endpoint uses PUT for updates
        headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (userProfileData) {
          // Check userType
          if (userProfileData.userType === "Club") {
            // Fetch club profile details
            const detailProfileClubData = await fetchDetailProfileClub(
              userProfileData.referencePvid
            );
            console.log("Club API Response: ", detailProfileClubData); // Log club API response
            setDetailProfileClub(detailProfileClubData);
          } else {
            // Fetch athlete profile details
            const detailProfileData = await fetchDetailProfile(
              userProfileData.referencePvid
            );
            console.log("Athlete API Response: ", detailProfileData); // Log athlete API response
  ...
  const fetchDetailProfile = async (referencePvid) => {
    try {
      ...
      localStorage.setItem("athletePvid", data.pvid);
  ...
  const fetchDetailProfileClub = async (referencePvid) => {
    try {
      const result = await axios.get(
        `${BASE_URL}Api/Club/GetById?Pvid=${referencePvid}`,
        {
          headers: await config(),
        }
      );
      ...
      console.log("Fetch Detail Profile Club API Response:", result.data);

      // Log the specific 'data.data' field returned by the API
      console.log("Club Data:", result.data.data);
  ...
  const updateProfile = async () => {
    try {
      ...
      const result = await axios.post(
        BASE_URL + `Api/AthleteProfile/EditById/${detailProfile.pvid}`,
        {
  ...
  const handleClubSubmit = async (event) => {
    ...
    try {
      ...
      const updatedProfileData = {
        ...
        IsActive: detailProfile.isActive,
        ClubPvid:
          listClub.find((club) => club.name === newClub)?.pvid ||
          detailProfile.clubPvid, // Use the pvid of the selected club, or the existing club's pvid
        Coach: detailProfile.coach,
        IsPublish: detailProfile.isPublish,
      };
  ...
  const handleClubInputChange = (field, value) => {
    setDetailProfileClub((prev) => ({ ...prev, [field]: value }));
  };
  const updateClubProfile = async () => {
    const requestBody = {
      Pvid: detailProfileClub.pvid, // Assuming 'pvid' is already part of the detailProfileClub object
      Name: detailProfileClub.name,
      Phone: detailProfileClub.phone,
    };
  
    try {
      const response = await fetch(BASE_URL + "Api/Club/ModifyById", {
        method: "PUT", // Assuming the endpoint uses PUT for updates
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        const data = await response.json();
        console.log("Club profile updated successfully:", data);
        toggleEditing(false); // Exit editing mode
      } else {
        console.error("Failed to update club profile:", response.statusText);
      }
    } catch (error) {
  ...
  return (
    <div>
      <div className="py-2">
        <h2 className="text-1xl text-gray-700 font-semibold uppercase">
          Account
        </h2>
      </div>
      ...
      {usertype !== "Athlete" && usertype !== "Member" &&
       usertype == "Club" &&
        detailProfileClub && (
          <div className="overflow-x-auto bg-white rounded-lg shadow">
            <table className="w-full whitespace-no-wrap bg-white overflow-hidden table-striped">
                ...
                <tr className="focus-within:bg-gray-200 overflow-hidden">
                  ...
                  <td className="border-t">
                    {editing ? (
                      <input
                        type="text"
                        value={detailProfileClub.phone || ""}
                        onChange={(e) => handleClubInputChange("phone", e.target.value)}
                        className="text-gray-700 px-2 py-2 flex items-center bg-gray-100 rounded w-full"
                      />
                    ) : (
                      <span className="text-gray-700 px-6 py-3 flex items-center">
                        {detailProfileClub.phone}
                      </span>
                    )}
                  </td>
                </tr>
        ...
        <table className="w-full whitespace-no-wrap bg-white overflow-hidden table-striped">
          {usertype !== "Club" && usertype !== "Member" && (
            <tbody>
                  ...
                  <div className="text-gray-700 px-6 py-3 flex items-center">
                    {editing ? (
                      <input
                        ...
                        onChange={(e) =>
                          handleChange("uniqueCode", e.target.value)
                        }
                        className="text-gray-700 px-2 py-2 flex items-center bg-gray-100 rounded w-full"
                      />
                    ) : (
                ...
                <td className="border-t">
                  <div className="text-gray-700 px-6 py-3 flex items-center">
                    {editing ? (
                      <input
                        type="date"
                        value={moment(detailProfile.birthDate).format(
                          "YYYY-MM-DD"
                        )}
                        onChange={(e) =>
                      ...
                      <span>
                        {moment(detailProfile.birthDate).format("DD MMM YYYY")}
                      </span>
                    )}
                  </div>
                </td>
                  ...
                  {isEditingClub ? (
                    <span className="text-gray-700 px-6 py-3 flex items-center">
                      <select
                        ...
                        className="text-gray-700 px-2 py-2 flex items-center bg-gray-100 rounded"
                      >
                        {listClub.map((club) => (
                  ...
                  {isEditingClub ? (
                    ...
                    <span className="text-gray-700 px-6 py-3 items-center">
                      <button
                        ...
                        className="bg-blue-500 hover:bg-blue-700 text-white py-1 px-6 rounded"
                      >
                        Change Club
                      </button>
                    </span>
              ...
              {submitError && (
                <div className="text-red-600 mt-2 px-6 py-3">{submitError}</div>
              )}
              {/* Coach */}
              {/* Coach Field */}
              <tr className="focus-within:bg-gray-200 overflow-hidden">
  ...
  <tbody>
    ...
    <tr className="focus-within:bg-gray-200 overflow-hidden">
      <td className="border-t">
        <button
          onClick={() => toggleEditing(!editing)} // Toggle edit mode
          className="bg-blue-500 hover:bg-blue-700 text-white py-1 px-4 rounded"
        >
          {editing ? "Cancel" : "Edit"}
        </button>
      ...
      <td className="border-t">
        {editing && (
          <button
            onClick={updateUserProfile} // Update the profile via API
            className="bg-green-500 hover:bg-green-700 text-white py-1 px-4 rounded"
          >
            Update
          </button>
